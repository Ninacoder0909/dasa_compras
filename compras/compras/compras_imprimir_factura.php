...
$sqlctotal = "select *,(select sp_numero_letras(total::numeric)) as total_letra from v_totales where id_compra = " . $_REQUEST['vidcompra'] . " order by 1";
$toti = consultas::get_datos($sqlctotal);
$sqlcompras = "select * from v_compras where id_compra = " . $_REQUEST['vidcompra'] . " order by 1";
...
$pdf->Cell(85, 1, 'T.S Electricidad e informática S.A', 0, 0, 'C', null, null, 1);

$pdf->SetFont('Times', 'B', 10);
$pdf->Cell(85, 1, '         COMPRA DE ARTICULOS', 0, 0, 'C');
$pdf->Ln(10);

$pdf->SetFont('Times', '', 12);
$pdf->Cell(95, 1, 'Dirección: Pitiantuta N°151 c/Mcal. Estigarribia', 0, 0, 'L');
$pdf->Cell(75, 1, 'Factura de Compra', 0, 0, 'C');
$pdf->Ln(5);
...
$pdf->SetFont('Times', '', 12);
$pdf->Ln(5);
$pdf->Cell(50, 1, 'Timbrado N°: ' . '', 0, 0, 'C');
$pdf->SetFont('Times', '', 10);
$pdf->Cell(2, 1, $rscompras[0]['timbrado'], 0, 0, 'L');
$pdf->Cell(29, 1, '   ' . '', 0, 0, 'C');
$pdf->Cell(50, 1, '   Factura N°: ', 0, 0, 'C');
$pdf->SetFont('Times', '', 10);
$pdf->Cell(70, 1, $rscompras[0]['nro_factura'], 0, 0, 'L');
$pdf->Ln(5);
$pdf->Cell(85, 1, 'Vigencia: ' . '01-01-2023', 0, 0, 'C');
...
$pdf->Cell(/*1*/90, /*2*/ 1, /*3*/ $rscompras[0]['fechac'], /*4*/ 0, /*5*/ 1, /*6*/ 'L', /*7*/ null, /*8*/ null, /*9*/ 1, /*10*/ null, /*11*/ null, /*12*/ null);
$pdf->Ln(3);
...
$pdf->SetFont('Times', '', 11);
$pdf->Cell(90, 1, $rscompras[0]['prv_ruc'], 0, 1, 'L');
...
$consultas = "select * from v_compras_detalle where id_compra=" . $_REQUEST['vidcompra'];
...
foreach ($detcompras as $report) {
    ...
    $pdf->Cell(22, 1, number_format(($report['precio']), 0, ',', '.'), 0, 0, 'R');
    $pdf->Cell(22, 1, number_format(($report['precio'] * $report['cantidad']), 0, ',', '.'), 0, 0, 'R');
    $pdf->Cell(20, 1, number_format(($report['iva5']), 0, ',', '.'), 0, 0, 'R');
    $pdf->Cell(17, 1, number_format(($report['iva10']), 0, ',', '.'), 0, 0, 'R');
...
$pdf->SetFont('Times', '', 10);
$pdf->Text(165, 235, 'Gs. ' . ($toti[0]['ivatotal']));

$pdf->SetFont('Times', 'B', 10);
$pdf->Text(18, 243, 'TOTAL GENERAL');
$pdf->SetFont('Times', '', 10);
$pdf->Text(165, 243, 'Gs. ' . ($toti[0]['total']));

$pdf->SetFont('Times', 'B', 10);
$pdf->Text(18, 251, 'TOTAL EN LETRAS');
$pdf->SetFont('Times', '', 10);
$pdf->Text(55, 251, 'Son Gs. ' .
    ucfirst(strtolower($toti[0]['total_letra'])));
