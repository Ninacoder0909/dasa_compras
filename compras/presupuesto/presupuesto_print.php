...
$presupuestos = consultas::get_datos("select * from v_presupuesto "
                . "where id_presupuesto=" . $_REQUEST['vidpedido'] . " order by id_presupuesto");
...
if (!empty($presupuestos)) {
  ...
  $pdf->Ln();
        $pdf->SetFillColor(255, 255, 255);
        $pdf->SetFont('', '', 10);
        $pdf->Cell(20, 5, $presupuesto['id_presupuesto'], 0, 0, 'C', 1);
            $pdf->Cell(40, 5, $presupuesto['suc_descri'], 0, 0, 'C', 1);
            $pdf->Cell(40, 5, $presupuesto['usu_nick'], 0, 0, 'C', 1);
            $pdf->Cell(40, 5, $presupuesto['id_pedido']." - ". $presupuesto['fechap'], 0, 0, 'C', 1);
            $pdf->Cell(40, 5, $presupuesto['prv_razon_social']." - ". $presupuesto['prv_ruc'], 0, 0, 'C', 1);
            $pdf->Cell(40, 5, $presupuesto['fecha'], 0, 0, 'C', 1);
            $pdf->Cell(40, 5, $presupuesto['estado'], 0, 0, 'C', 1);
            $pdf->Cell(40, 5, $presupuesto['pres_total'], 0, 0, 'C', 1);
//           
        $pdf->Ln();
        $pdf->Ln();

        $detalles = consultas::get_datos("select * from v_detalle_presupuesto "
                        . "where id_presupuesto=" . $presupuesto['id_presupuesto'] . " order by id_presupuesto");
        if (!empty($detalles)) {
      ...
      $pdf->Cell(20, 5, $detalle['id_presupuesto'], 1, 0, 'C', 1);
