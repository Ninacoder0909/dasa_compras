...
if ($_REQUEST['vopcion'] == '1') {
    ...
    $compras = consultas::get_datos("select * from v_compras where fechac BETWEEN $fechainicial AND $fechafinal");
    if (!empty($compras)) {
        foreach ($compras as $pedido) {
            ...
            $pdf->SetFillColor(255, 255, 255);
            $pdf->SetFont('', 'B', 12);
            $pdf->Cell(50, 5, $pedido['nro_factura'], 1, 0, 'C', 1);
            ...
            $pdf->SetFillColor(180, 180, 180);
            $pdf->Cell(190, 5, '         ', 0, 0, 'C');
            $pdf->Cell(30, 5, 'CONDICION', 1, 0, 'C', 1);

            $pdf->SetFillColor(255, 255, 255);
            $pdf->SetFont('', 'B', 12);
            $pdf->Cell(50, 5, $pedido['condicion'], 1, 0, 'C', 1);
            ...
            $pdf->SetFont('', '', 12);
            $pdf->Cell(20, 5, '         ', 0, 0, 'C', 1);
            $pdf->Cell(40, 5, $pedido['id_compra'], 1, 0, 'C', 1);
            $pdf->Cell(100, 5, $pedido['compra_fecha'], 1, 0, 'C', 1);
            ...
            $pdf->Cell(290, 3, 'DETALLE DE COMPRA NRO.:  ' . $pedido['id_compra'], 0, 0, 'C', 0);
            $pdf->Ln();
            $pdf->Ln();

            $detalles = consultas::get_datos("select * from v_compras_detalle where id_compra=" . $pedido['id_compra'] . " order by id_compra");
            if (!empty($detalles)) {
                $pdf->SetFillColor(255, 255, 255);
                $pdf->SetFont('', 'B', 12);
                ...
                foreach ($detalles as $detalle) {
                    ...
                    $pdf->Cell(30, 5, $detalle['exentas'], 1, 0, 'C', 1);
...
if ($_REQUEST['vopcion'] == '2') {
    ...
    $compras = consultas::get_datos("select * from v_compras where estado = $estados");
    if (!empty($compras)) {
        foreach ($compras as $pedido) {
            //columnas
            $pdf->Cell(0, 0, "REPORTE DE COMPRAS", 0, 1, 'C');
