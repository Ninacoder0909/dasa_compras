...
if ($_REQUEST['vopcion'] == '1') {
    //ME TRAE POR ESTADO
    $proveedor = $_REQUEST['vprov'];
    $pedidoss = consultas::get_datos("SELECT * FROM v_cuentas WHERE prv_cod = $proveedor");
    if (!empty($pedidoss)) {
        foreach ($pedidoss as $pedidos) {
            ...
            $pdf->Cell(290, 3, 'DETALLE DE CUENTAS NRO.:  ' . $pedidos['id_cuenta'], 0, 0, 'C', 0);
            $pdf->Ln();
            $pdf->Ln();
            ...
            $pdf->Cell(20, 7, '', 0, 0, 'C');
            $pdf->Cell(110, 7, 'VENCIMIENTO', 1, 0, 'C', 1);

            $pdf->Cell(60, 7, 'MONTO', 1, 0, 'C', 1);
            $pdf->Ln(); //salto
            ...
            $pdf->Cell(20, 7, '', 0, 0, 'C');
            $pdf->Cell(110, 7, $pedidos['vencimiento'], 1, 0, 'C', 1);

            $pdf->Cell(60, 7, $pedidos['monto'], 1, 0, 'C', 1);
            $pdf->Ln();
...
if ($_REQUEST['vopcion'] == '2') {
    ...
    $pedidoss = consultas::get_datos("SELECT * FROM v_cuentas WHERE estado = $id_estado ORDER BY id_cuenta");
    if (!empty($pedidoss)) {
        foreach ($pedidoss as $pedidos) {
            $pdf->Cell(0, 0, "REPORTE DE CUENTAS", 0, 1, 'C');
            $pdf->SetFont('', 'B', 14);
