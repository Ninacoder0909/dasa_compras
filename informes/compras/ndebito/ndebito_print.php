...
if ($_REQUEST['vopcion'] == '1') {
    ...
    $compras = consultas::get_datos("select * from v_ndebito where fecha_sistema BETWEEN $fechainicial AND $fechafinal");
    if (!empty($compras)) {
        foreach ($compras as $pedido) {
            //columnas
            $pdf->Cell(0, 0, "REPORTE DE NOTAS DE DEBITO", 0, 1, 'C');
            $pdf->SetFont('', 'B', 12);
            $pdf->SetFillColor(255, 255, 255);
            ...
            $pdf->SetFont('', '', 12);
            $pdf->Cell(20, 5, '         ', 0, 0, 'C', 1);
            $pdf->Cell(40, 5, $pedido['id_debito'], 1, 0, 'C', 1);
            ...
            $pdf->Cell(290, 3, 'DETALLE DE NOTA DE DEBITO NRO.:  ' . $pedido['id_debito'], 0, 0, 'C', 0);
            $pdf->Ln();
            $pdf->Ln();

            $detalles = consultas::get_datos("select * from v_ndebito_detalle where id_debito=" . $pedido['id_debito'] . " order by id_debito");
            if (!empty($detalles)) {
                $pdf->SetFillColor(255, 255, 255);
                $pdf->SetFont('', 'B', 12);
                ...
                $pdf->Cell(30, 5, 'PAGO EXTRA', 1, 0, 'C', 1);
                ...
                foreach ($detalles as $detalle) {
                    ...
                    $pdf->Cell(70, 5, $detalle['descripcion'], 1, 0, 'C', 1);
                    if ($detalle['pro_cod'] > 0) {
                        ...
                        $pdf->Cell(30, 5, $detalle['pago_extra'], 1, 0, 'C', 1);
                ...
                $pdf->SetFillColor(255, 255, 255);
                $pdf->Cell(100, 5, $pedido['total_debito'], 1, 0, 'C', 1);
                $pdf->Ln();
                $pdf->Ln();
                $pdf->Ln();
                $pdf->Ln();
...
if ($_REQUEST['vopcion'] == '2') {
    $vestado = $_REQUEST['vciudad'];
    $compras = consultas::get_datos("select * from v_ndebito where estado = $vestado");
    if (!empty($compras)) {
        foreach ($compras as $pedido) {
            //columnas
            $pdf->Cell(0, 0, "REPORTE DE NOTAS DE DEBITO", 0, 1, 'C');
