...
if ($_REQUEST['vopcion'] == '1') {
    ...
    if (!empty($pedidoss)) {
        foreach ($pedidoss as $pedidos) {
            ...
            $pdf->Cell(20, 7, $pedidos['id_ordenc'], 0, 0, 'L');
            $pdf->Cell(50, 7, '', 0, 0, 'C');
            $pdf->Cell(50, 7, '', 0, 0, 'C');
            $pdf->Cell(70, 7, '', 0, 0, 'C');
            ...
            $pdf->SetFont('', 'N', 14);
            $pdf->Cell(30, 7, $pedidos['orden_fecha'], 0, 0, 'L');
            $pdf->Cell(20, 7, '', 0, 0, 'C');
            $pdf->Cell(50, 7, '', 0, 0, 'C');
            $pdf->Cell(50, 7, '', 0, 0, 'C');
            $pdf->Cell(44, 7, '', 0, 0, 'C');
            $pdf->SetFont('', 'B', 14);
            $pdf->Cell(22, 7, 'PROVEEDOR:', 0, 0, 'C');
            $pdf->Cell(50, 7, $pedidos['prv_razon_social'], 0, 0, 'C');

            $pdf->Ln();
            $pdf->Ln();

            $pdf->SetFont('times', 'B', 12);
            $pdf->Cell(290, 3, 'DETALLE DE ORDEN NRO.:  ' . $pedidos['id_ordenc'], 0, 0, 'C', 0);
            $pdf->Ln();
            $pdf->Ln();

            $detalles = consultas::get_datos("select * from v_orden_detalle where id_ordenc=" . $pedidos['id_ordenc']);
            if (!empty($detalles)) {
                $pdf->SetFont('', 'B', 13);
...
if ($_REQUEST['vopcion'] == '2') {
    ...
    $pedidoss = consultas::get_datos("SELECT * FROM v_orden_de_compra WHERE estado = $id_estado ");
    if (!empty($pedidoss)) {
        foreach ($pedidoss as $pedidos) {
            $pdf->SetFont('', 'B', 14);
            //columnas
...
$pdf->Output('reporte_orden_dasa.pdf', 'I');
