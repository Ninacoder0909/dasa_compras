...
if ($_REQUEST['vopcion'] == '1') {
    ...
    if (!empty($pedidoss)) {
        foreach ($pedidoss as $pedidos) {
            ...
            $pdf->Cell(20, 7, $pedidos['id_presupuesto'], 0, 0, 'L');
            $pdf->Cell(50, 7, '', 0, 0, 'C');
            $pdf->Cell(50, 7, '', 0, 0, 'C');
            $pdf->Cell(70, 7, '', 0, 0, 'C');
            ...
            $pdf->SetFont('', 'N', 14);
            $pdf->Cell(30, 7, $pedidos['presu_fecha'], 0, 0, 'L');
            $pdf->Cell(20, 7, '', 0, 0, 'C');
            $pdf->Cell(50, 7, '', 0, 0, 'C');
            $pdf->Cell(50, 7, '', 0, 0, 'C');
            $pdf->Cell(44, 7, '', 0, 0, 'C');
            ...
            $pdf->Cell(290, 3, 'DETALLE DE PEDIDO NRO.:  ' . $pedidos['id_presupuesto'], 0, 0, 'C', 0);
            $pdf->Ln();
            $pdf->Ln();

            $detalles = consultas::get_datos("select * from v_detalle_presupuesto where id_presupuesto=" . $pedidos['id_presupuesto']);
...
if ($_REQUEST['vopcion'] == '2') {
    ...
    $pedidoss = consultas::get_datos("SELECT * FROM v_presupuesto WHERE estado = $id_estado ");
    if (!empty($pedidoss)) {
        foreach ($pedidoss as $pedidos) {
            $pdf->SetFont('', 'B', 14);
            //columnas
